<script>
  ...
  onMount(() => {
    ...
    el.addEventListener('pointerdown', (e) => {
      ...
      if (!vectorTarget && !DITHER_TOOLS.includes(globalState.tool.current?.name)) return
      control.setPointerCapture(e.pointerId)
      const startX = e.clientX
      const startY = e.clientY
      const target = vectorTarget ?? globalState.tool.current
      ...
      const onMove = (ev) => {
        ...
        if (vectorTarget) renderCanvas(vectorTarget.layer, true)
...
>
  <div class="header dragger">
    ...
    Dither Pattern
    <button type="button" class="close-btn" data-tooltip="Close"></button>
  </div>
  <div class="collapsible">
    <div class="dither-controls">
      ...
      ></button>
      <div class="dither-offset-control-wrap"></div>
    </div>
    <div class="build-up-steps" style="display: none">
      <span class="build-up-steps-label">Build-Up Steps</span>
      <div class="build-up-mode-selector">
        <button type="button" class="build-up-mode-btn selected" data-mode="custom" data-tooltip="Custom build-up steps">Custom</button>
        <button type="button" class="build-up-mode-btn" data-mode="2x2" data-tooltip="4 steps from a 2x2 Bayer Matrix">2×2</button>
        <button type="button" class="build-up-mode-btn" data-mode="4x4" data-tooltip="16 steps from a 4x4 Bayer Matrix">4×4</button>
        <button type="button" class="build-up-mode-btn" data-mode="8x8" data-tooltip="64 steps from an 8x8 Bayer Matrix">8×8</button>
      </div>
      <div class="build-up-step-slots"></div>
      <button
        type="button"
