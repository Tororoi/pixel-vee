...
const renderVectorElement = (vector) => {
  ...
  vectorElement.style.background = isSelected
    ? 'rgb(0, 0, 0)'
    : 'rgb(51, 51, 51)'
...
export function renderVectorSettingsToDOM(domVector) {
  ...
  const secColor = vector.secondaryColor ?? {
    r: 0,
    g: 0,
    b: 0,
    a: 0,
 ...
 */
const drawOnThumbnailContext = (
  vector,
  isSelected,
  ctx = canvas.thumbnailCTX,
) => {
  ...
  switch (vector.vectorProperties.tool) {
    ...
    case 'curve': {
      ...
      } else if (vector.modes.quadCurve) {
 ...
 */
const createVectorDitherPatternSVG = (
  pattern,
  vector,
  offsetX = 0,
  offsetY = 0,
...
export function updateVectorDitherControls(vector) {
  ...
  if (twoColorBtn)
    twoColorBtn.classList.toggle('selected', vector.modes?.twoColor ?? false)
  ...
  const wrap = container.querySelector('.dither-offset-control-wrap')
  if (wrap)
    applyVectorDitherOffsetControl(wrap, effectiveOffsetX, effectiveOffsetY)
}
...
export function highlightVectorDitherPattern(patternIndex) {
  const grid = dom.vectorDitherPickerContainer?.querySelector('.dither-grid')
  if (!grid) return
