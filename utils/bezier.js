...
export function plotCubicBezier(x0, y0, x1, y1, x2, y2, x3, y3) {
  ...
  function plotCubicBezierSeg(x0, y0, x1, y1, x2, y2, x3, y3, color) {
    ...
    if (xa === 0 && ya === 0) {
      ...
      return plotQuadBezierSeg(x0, y0, sx, sy, x3, y3, `rgba(0,255,150,255)`)
    }
    ...
    do {
      ...
      for (pxy = xy, fx = fy = f; x0 !== x3 && y0 !== y3; ) {
        ...
        count += 1
        if (count > 100) {
          console.log("for do", x3, y3, x0, y0, fx, fy, f, count)
        }
        if (count > 200) {
          break
        }
        ...
        if (count > 100) {
          console.log("incx", 2 * fx <= f)
          console.log("incy", 2 * fy <= f)
        }
        if (2 * fx <= f) {
          x0 += sx
          fx += f
