...
export function debugPlotCubicBezier(x0, y0, x1, y1, x2, y2, x3, y3, maxSteps) {
  ...
  function plotCubicBezierSeg2(x0, y0, x1, y1, x2, y2, x3, y3, color) {
    ...
    // if (xa === 0 && ya === 0) {
    //   // quadratic Bezier
    //   //sx, sy represent new middle control point
    //   sx = Math.trunc((3 * x1 - x0 + 1) / 2)
    //   sy = Math.trunc((3 * y1 - y0 + 1) / 2)
    ...
    //   f = Math.trunc(ex > 0 ? 1 : Math.sqrt(1 + 1024 / x1)) //calculate resolution
    //   //increase resolution
    //   ab *= f
    //   ac *= f
    //   bc *= f
    ...
    //       if (pxy == 0) if (dx > xy || dy < xy) break outerLoop /* confusing */
    //       if (pxy == 1) if (dx > 0 || dy < 0) break outerLoop /* values */
